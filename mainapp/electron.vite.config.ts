...
import extendsHonoUserConfig from 'extends-hono/userConfig'
import localCodexUserConfig from 'chatgpt-com-tocodex/userConfig'

delete process.env.ELECTRON_RUN_AS_NODE

const { host, viteHmrPort: port } = packageJson
...
for (const scope of ['preload', 'renderer'] as const) {
  ...
  for (const [owner, configuration] of [
    ...
    ['extends-hono', extendsHonoUserConfig as UserConfig],
...
export default defineConfig(
  mergeConfig(
    mergeConfig(hostUserConfig, extendsHonoUserConfig),
    localCodexUserConfig
  )
)
