...
type UserRoute = "chat" | "image" | "research";
...
const userRoutes: {
  ...
  { route: "chat", label: "聊天", icon: <MessageOutlined /> },
  { route: "image", label: "图片", icon: <PictureOutlined /> },
  { route: "research", label: "深度研究", icon: <ReadOutlined /> },
];
...
export default function User({ userRoute }: UserProps) {
  ...
  useEffect(() => userActions.state.connect(), [userActions]);

  useEffect(() => {
    userActions.mcp.toolsLoad();
  }, [userActions]);

  const treeData = useMemo(() => treeDataFromNodes({ nodes: user.state?.topic?.nodes || [], topicId: user.state?.topic?.topicId || "" }), [user.state]);
  ...
  const speechToggle = () => {
    ...
    if (!SpeechRecognition) {
      userActions.errorText.set("当前环境不支持语音输入");
      return;
    }
    ...
    speechRecognition.onerror = (event) => {
      userActions.errorText.set(event.error ? `语音输入失败：${event.error}` : "语音输入失败");
    };
  ...
  const routePathGo = (nextUserRoute: UserRoute) => {
    navigate(nextUserRoute === "chat" ? "/" : `/${nextUserRoute}`);
  ...
  const inputPanel = (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      <Input.TextArea
        ...
        placeholder={user.state?.canUseChatgpt ? placeholderText : "等待 admin 允许使用"}
    ...
    >
      {canUseChatgpt ? (
        ...
        <div style={{ position: "relative", height: "100vh", overflow: "hidden", background: "#06070a", color: "#fff" }}>
          ...
          <div style={{ position: "relative", zIndex: 1, display: "grid", placeItems: "center", height: "100%", fontSize: "clamp(32px, 6vw, 72px)", letterSpacing: "-.04em", pointerEvents: "none" }}>等待授权聊天</div>
        </div>
      )}
