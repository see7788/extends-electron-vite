...
const userAppStore = {
  hostWindow: void 0,
  hostPreloadPath: node_path.join(rootDir, "userapp", "electron-preload", "out", "index.js"),
  userAppWebUrl: "http://127.0.0.1:8810/"
};
function startShareApiServer() {
  const appApi = new Hono2();
  appApi.get("/api/share/ping", (context) => {
    return context.json({ ok: true });
  });
  appApi.get("/api/share/ready", (context) => {
    return context.json({ ready: true, url: userAppStore.userAppWebUrl });
  });
  serve({
    fetch: appApi.fetch,
    port: 8820
...
function userAppWindowCreate() {
  userAppStore.hostWindow = new electron.BrowserWindow({
    width: 1300,
    height: 860,
    title: "UserApp Host",
    webPreferences: {
      preload: userAppStore.hostPreloadPath,
  ...
  userAppStore.hostWindow.loadURL(userAppStore.userAppWebUrl);
}
function appReady() {
  startShareApiServer();
  userAppWindowCreate();
...
function appActivate() {
  if (!userAppStore.hostWindow || userAppStore.hostWindow.isDestroyed()) userAppWindowCreate();
}
function appAllClosed() {
  if (process.platform !== "darwin") electron.app.quit();
}
electron.app.whenReady().then(appReady);
electron.app.on("activate", appActivate);
electron.app.on("window-all-closed", appAllClosed);
