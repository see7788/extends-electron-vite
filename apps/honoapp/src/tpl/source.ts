      ...
      sections: [
        {
          title: "正向业务链",
          items: [
            "先用一句话确定真实结果，核对生产者、消费者、owner、公开语义、消费责任和验收能够同时成立，再确定唯一业务入口。入口可以是 class 公开主方法、普通函数、高阶函数、模块方法、route handler、store action、命令或事件处理器。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "源码 tree 使用 `├──`、`└──`、`│` 连线，以真实目录和文件为骨架；对象目录下第一级是文件名，文件节点本身就是生产者，不写 `生产` 注释。大写 Class 文件由文件名直接表达唯一同名 Class，禁止再列 `default: class Name`、`Name: class` 或其他重复声明节点；非 Class 入口只有在导出形态无法由文件名确定时才显式写 `default` 或命名入口。",
            "文件节点第二级只列边界外真实消费者可直接使用的公开成员及其类型或完整签名；仓库内部字段、私有 helper、实现步骤和无独立消费价值的类型细节不进入 tree，`store: StoreApi` 等边界已经清楚时不展开整份状态结构。找不到真实消费者的公开成员先按能力准入从源码删除，文档示例不构成消费者证据。",
            "调用关系只标在实际发起调用的公开方法、函数或命令下面，统一写成 `调用 File.member()`；第一级文件名保留真实扩展名，调用引用省略 `.ts`，禁止在生产者或文件节点重复写消费方。多个调用较短时写在同一调用节点，过长时在 `调用` 子节点下逐项展开。",
            `tree 中的方法名、形参和返回类型必须与源码一致，并遵守 ${nodes.codeStyle} 的作用域短命名与对象形参规则；一个变化参数直接传值，两个及以上变化参数使用对象，长对象签名按字段换行，不为保持单行压缩类型。`,
          ...
          code: {
            ...
            content: [
              ...
              "│       └── 调用 public.execute()",
              "├── Gateway.ts",
              "│   ├── readonly state: { host: string; port: number }",
              "│   └── isRunning(): Promise<typeof this.state>",
              "│       └── 调用 Process.isRunning()、public.execute()",
              "└── Publisher.ts",
              "    └── publish(",
              "          options: {",
              "            sourcePath: string;",
