    ...
    [nodes.codeStyle]: {
      ...
      sections: [
        ...
        },
        {
          title: "最小作用域与真实实现",
          items: [
            "先在当前消费点直接实现。只有独立证据已经证明存在多个同语义真实消费点，或当前需求已经实际产生必须独立维护的状态、生命周期或不变量时，才评估提升为模块、函数、类型、class 或文件；不得以将来可能复杂、理论生命周期或本轮为证明抽象而制造的调用作为依据。",
        ...
        },
        {
          ...
          items: [
            ...
            "多个具体实现代码相似只允许评估复用，不证明应提升到基类。只有共同性独立于拟新增的基类成员已经成立，且所有受影响实现具有相同语义、owner、变化原因和共同不变量时，才可提出修改基类；具体协议的代理、类型、校验、生命周期、便利方法和状态留在具体实现的最小作用域。",
    ...
    [nodes.parentWorkflow]: {
      ...
      sections: [
        ...
        },
        {
          title: "工作者准备与派工",
          items: [
            "仅在当前会话第一条方先生消息时，且当前运行时实际提供 watcher 与 environment.check、方先生没有要求不用工作队或全部由 parent 完成时，parent 才启动唯一的会话级 watcher并调用 environment.check；返回空数组则继续，返回非空数组由 parent 记录真实错误并停止依赖该环境的任务。watcher 不执行环境检查，只监督 parent 是否跳过已适用的 environment.check、忽略非空错误或在未解除时继续依赖该环境。普通追问、澄清和连续对话不得重复启动；方先生在执行中明确取消工作队时，立即停止仍在运行的 watcher 与具体工作者。",
            ...
            "使用工作队时，parent 只承担领导职责：理解方先生对话、建立/调整任务树、比较工作者反馈与验收、更新任务树内容与状态、派工、接收 watcher bug、重排和汇报，不接管已经派出的业务实现。方先生明确要求不用工作队或全部由 parent 完成时，parent 不创建具体工作者并亲自完成调查、实现、验证和收尾；当前运行时没有可用具体工作者时也由 parent 继续完成，不把工作者缺失当作任务阻塞。",
          ],
        },
        {
          title: "任务树准备",
        ...
        },
        {
          ...
          orderedItems: [
            ...
            "快速使用只展示完成核心结果所需的最短公开调用链。通用初始化样板可以压缩，但示例引用的命令、入口、实例和必要配置，必须在当前快速使用段中定义，或在紧邻位置给出明确的创建方式、导入路径、配置位置或前置命令。禁止使用未定义且无来源说明的占位符冒充“最短可运行”。",
        ...
        },
        {
          ...
          items: [
            ...
            "tree 中的公开能力包括包或模块入口导出、实例可调用成员和明确提供给继承方的 protected 扩展点；排除 private、`@internal` 和只供同包实现协作的成员。",
            "能力提供方只写提供什么，不写哪里消费了它；消费方如果依赖其他公开能力，才在自身子节点说明消费链路。",
            "消费链路使用“调用、继承、组合或转出 `<相对文件>` 的 `<公开符号或方法>`”描述，只写到第一个公开边界，不展开私有中间过程。",
            "公开能力已在提供方文件展开后，继承、组合或转出它的消费方只说明来源关系，不重复复制提供方的全部成员。",
            "每个目录、文件、能力和注释必须在同一行完整表达；禁止把一个 tree 节点的名称或注释折到下一行。",
