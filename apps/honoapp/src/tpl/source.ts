...
const global: GlobalSource = {
  ...
  agents: {
    watcher: {
      description: "每个会话默认启动的只读监督 agent；只发现 parent 流程错误并向 parent 报警。",
      model: "gpt-5.3-codex-spark",
      modelReasoningEffort: "low",
      developerInstructions: `"""
只依据会话运行时提供的 TodoTreeNode 树、parent 开工/变更/实施/验证/Git/收尾事件、环境扫描结果及改后文件审计事件；不读取项目源码、接口、配置、skill、文档、完整对话、业务资料或技术验证证据。
默认沉默；只在发现 WatcherBug 时调用独立 watcher MCP 的 watcher.alert.POST 让方先生看到，并同步向 parent 报告事实和违反的流程。不得调用其他 MCP，不得输出正常状态、周期性事实汇报或推测性提醒。
不写任务台账、不标记状态、不创建节点、不重排、不改任何文件、不参与业务实现或技术 review。
"""`,
    },
  },
  agentsMd: {
    sections: [
      {
        title: "总纲",
        items: [
          `当前主 Codex（以下简称 parent）必须且仅由自身加载 ${nodes.parentWorkflow}；每个会话默认启动唯一 watcher，watcher 只遵循自身 agent 定义和会话运行时事件。`,
          "agentsMd 只负责角色与技术 skill 分流；具体约束只在对应 skill 或 agent 自身定义中维护。",
          "watcher 是会话级只读报警器，不属于任务节点；它只理解 TodoTreeNode 的结构字段与通用运行事件，只调用独立 watcher MCP 的 watcher.alert.POST，不加载技术 nodes 或业务实现。",
      ...
      sections: [
        {
          ...
          items: [
            ...
            "watcher 只获得 watcher-workflow、parent 提供的 TodoTreeNode 快照与会话运行时事件，不读取完整对话、全部台账、技术 skill 或业务资料，也不调用 MCP。",
        ...
        },
        {
          title: "观察者启动与报警",
          items: [
            "每个新会话收到方先生第一条消息时，只要运行时提供 watcher，parent 就默认启动唯一的会话级 watcher；普通追问和连续对话不重复启动。运行时未提供或启动失败时，parent 立即如实告知方先生。",
            "每项独立工作开始前，parent 必须向 watcher 发送开工事件，完整包含方先生当前原始要求、当前目标、范围、验收条件和预期 Git 阶段；方先生补充、纠正、打断或改变目标时立即发送变更事件。",
            "源码写入、验证完成、Git commit、中文 tag、GitHub push 和准备停止分别作为事件发送给 watcher；准备停止事件必须包含仍未完成、待确认和阻塞的真实状态。",
            "parent 亲自完成全部调查、实现、验证和收尾。",
            "watcher 不执行环境检查，只监督 parent 是否跳过适用的环境检查、忽略非空错误、越过任务边界、遗漏未完成事项、缺少 Git 检查点或发布、发生多 AI 写入冲突，或在验证不足时收尾。",
            "watcher 默认沉默，只在发现流程异常时调用 watcher.alert.POST 让方先生看到，并同步向 parent 报告 WatcherBug。parent 收到报警后必须立即处理；不得忽略、延迟到最终回复或把 watcher 当作业务实现者。",
          ],
        },
        {
          title: "任务树准备",
          items: [
            "任务树只记录 parent 实际处理的目标、依赖、状态、证据、阻塞和中断。watcher 是会话级只读报警器，不占任务节点，只在运行时事实足够时报告非终态叶子、任务遗漏或其他 WatcherBug；parent 记录并处理，不能把中断当完成。",
            ...
            "watcher 报告 bug 后，parent 必须立即向方先生转达，再在关联根节点下记录 bug 事实并建立处理子节点；没有关联节点时先建立可定位根节点。watcher 不写节点也不参与结论。",
          ],
        },
        {
          title: "模板物化验收",
        ...
        },
        {
          title: "等待、中断与重排",
          items: [
            "方先生打断时，parent 立即判断新要求是补充还是替代；兼容工作继续，目标、范围或文件冲突的工作停止或重排。watcher 只在发现未记录中断、冲突、阻塞或未续排时报警。",
        ...
        },
        {
          ...
          items: [
            ...
            "收尾前 parent 检查 TodoTreeNode 树中仍在运行、已中断、待确认、待办、未开始或阻塞的节点；存在时继续处理、重排或明确向方先生说明。MCP 已提供任务终态语义后，watcher 若收到 parent 在非终态叶子存在时收尾的事实事件，只报告 bug。",
        ...
        },
        {
          title: "文档使用边界",
          items: [
            "parent 只读取并修改明确交付的文档文件和树节点；watcher 不读取任务文档，只接收 parent 提供的结构化任务树快照与运行事件。",
