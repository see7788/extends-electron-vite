      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "任务存在要求冲突、必要生产者缺失、契约未定义、授权不足，或方案会改变目标结果、公开契约、调用方责任、数据处理、外部副作用或不可逆范围时，parent 不以抛出错误代替分析，也不得自行补充业务定义；按结论、根因与直接证据、影响范围、推荐方案、待决事项的顺序向方先生说明并标为 `[?]`，停止依赖待决事实的实现。既定要求和事实已经唯一确定答案时，parent 直接作出最小决定并说明依据，不把普通实现细节转交方先生。",
        ...
        },
        {
          ...
          items: [
            ...
            "需要方先生决定时使用 `[?]`，写明待确认事实、直接证据、影响范围、推荐方案和理由；parent 不得用默认值、兜底、兼容或自行新增分支替代该决定。确认后再进入 `[ ]` 或 `[~]`，不依赖该决定的任务继续。",
