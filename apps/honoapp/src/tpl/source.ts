    ...
    [nodes.codeStyle]: {
      ...
      sections: [
        ...
        },
        {
          title: "生产者完整性准入",
          items: [
            "实现任何消费者前，先逐项列出该业务需要消费的数据，并把每项数据定位到唯一切片、具体生产者和正式公开成员。只有全部数据已经由生产者完整交付，才允许编写消费者。任意数据缺失、形态不完整或需要消费者理解生产步骤时，立即停止消费者路线：当前任务已授权生产者时回到生产者补齐并真实验证；未授权时按全局错误与决策规则提交缺口。",
        ...
        },
        {
          title: "最小作用域与真实实现",
          items: [
            "函数、方法和文件只按完整生产场景或消费场景确定边界，代码行数永远不构成拆分证据。一个场景无论多少行都先保持语义内聚；使用正常换行、局部变量、代码块、分支和必要注释组织阅读。只有真实出现独立项目数据、生产者、业务消费规则、状态、生命周期或不变量时才拆分，且只拆出对应的少量完整场景。",
    ...
    [nodes.codeClassStyle]: {
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            `Class 只能承载已经由 ${nodes.codeStyle} 确定的完整生产场景或消费场景。实现从切片数据或业务结果开始：先确定谁生产或谁消费、公开主方法怎样完整交付结果，再向方法内部完成具体实现；禁止从抽象契约、基类、工具 Class、技术步骤或成员骨架开始向上拼装业务。`,
