      ...
      sections: [
        {
          ...
          items: [
            ...
            "一切实现都先受一句精确的话统领；项目、入口业务、生产切片和 dataAction 在各自作用域都必须能用一句话完整说明要让哪份 data 成为什么可消费、可验证的结果。这不是字数或复杂度限制，而是单一、闭合的目的契约：任意复杂度只向下递归为更多成套生产切片，不向横向扩散为模糊职责、通用 helper、提前抽象或预留能力。无法形成这句话时边界尚未成立，禁止写代码。",
            "目的必须精确到边界外结果、真实消费者和验收条件，才具备实施资格；目的仍模糊时无法确定所需 data、dataAction 和生产者，也就不可能按需产生可执行项目。此时只继续查明目的，不得用脚手架、示例入口、通用能力、预留字段或猜测实现冒充项目进度。目的精确后，所需 data 与精确生产者必然能从结果逐项反查，生产能力不得多于或少于该结果的真实需要。",
            "实施方向固定为 `目的 → 成品 data → index/最终入口的完整类型与结果变量 → 递归生产切片（data + dataAction）→ return 完整结果`。类型可以一次表达整个项目成品的数据和公开方法；任何文件、Class、方法、helper、action、配置层或 export 如果不在这条路径上，就没有出生资格，不得先构思代码结构再把它接到目的上寻找用途。",
        ...
        },
        {
          ...
          items: [
            ...
            "data、数据、信息和运行时状态在本模型中是同一种项目数据，不因持久化位置、存活时间、传输形式或技术载体另建平行体系；它们都按真实语义进入数据切片，并接受相同的生产者、owner、消费和验收规则。",
            ...
            "可扩展的信息分类固定按三份并列数据表达：`信息`、`tag`、`信息_tag`。`信息` 与 `tag` 各自成为成套的 `data + dataAction`，`信息_tag` 作为独立关系数据及其 dataAction 表达 `n:n`；一条信息可以关联任意多个 tag，一个 tag 也可以关联任意多条信息。新增、删除或重组分类只生产 tag 与关系数据，不修改信息字段结构，也不在信息表或大对象中追加 `type`、`status`、`mode`、`enabled`、`source`、分类布尔值或大量可选字段模拟分类。消费者跨三份切片自由查询和组合，字段只描述本切片内部的真实事实。",
            "代码抽象本质上也是对已经真实存在的信息、生产场景或消费场景按需生产 tag 和关系数据。信息只因当前目的或真实消费者需要而产生，tag 只因已有对象出现真实归类需要而产生；不得预先创建未来可能有用的数据、字段、状态、类型、interface、基类、方法、能力目录、分类体系或空关系等待业务填充。抽象从具体事实归纳，只改变 tag 与关系数据，不反向改造原信息来证明抽象成立。",
            ...
            "生产者必须完整交付由精确目的反查出的单一 data 能力；一个对象即使被命名为 producer、service 或 manager，只要它仅调用多个生产者、安排执行顺序或组合结果，却没有创建并持续维护自己的独立 data，就属于业务消费者。套娃调用不能把业务编排变成生产者，也不得把这类消费者藏入生产切片。",
        ...
        },
        {
          title: "结果数据递归执行算法",
          orderedItems: [
            "先写一句精确目的，同时写明边界外真实消费者、最终 result data 和真实验收条件；任一项说不清时停止源码路线，只继续查明目的。",
            "用完整类型一次定义最终 result，并在入口保留唯一完成出口；不得使用 `Partial`、可选字段、空对象、类型断言、默认值或多套候选结果降低完整性。",
            "逐项检查 result 所需 data：已经由真实生产者交付的标为可直接消费，尚未成立的才进入生产切片；不得把方法、文件或技术步骤列成 data。",
        ...
        },
        {
          ...
          items: [
            ...
            "生产者尚未由 `data + dataAction + owner` 明确时，helper、工具对象、公共方法和 export 都没有出生资格；不得用无主 helper 暂存生产步骤，再用 export 扩散可达性来推迟生产边界决定。只服务一个 dataAction 的实现留在该生产者内部，只服务一个具体业务的组合留在消费者内部；只有独立的真实跨边界消费者已经存在，才进入公开导出准入。",
        ...
        },
        {
          ...
          items: [
            ...
            "入口先定义最终要 return 的完整结果数据；需要逐步成立时使用一个带完整类型的 `let result`，不使用 `Partial`、类型断言、空对象、默认值或可选字段伪装完成。实现只回答怎样让 result 所需的每份 data 真实成立；缺哪份 data，才递归建立对应的 `data + dataAction`。完整 result 通过真实验收并从边界 return，就是当前一句话目的完成。",
            ...
            "入口反复用 `if` 检查缺值、标志组合、生产状态、兜底来源或上一技术步骤是否成功时，默认判定为 result 尚未定义完整、多个切片被压入同一模型，或生产者没有交付确定数据；先返回 result 和生产切片修正。只有契约真实存在的业务状态才保留分支，并穷尽每个合法状态，使所有成功分支都交付同一完整结果。",
