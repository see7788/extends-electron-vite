      ...
      sections: [
        {
          ...
          items: [
            ...
            "实施方向固定为 `目的 → 具象结果 → index/最终入口的数据与方法 → 结果所需切片 → 唯一纯生产者 → 具体实现`。任何文件、Class、方法、helper、action、配置层或 export 如果不在这条路径上，就没有出生资格；不得先构思代码结构，再把它接到目的上寻找用途。",
        ...
        },
        {
          title: "生产者完整性准入",
          items: [
            "生产者完整性准入不表示先按构思批量创建生产者文件和方法。新增成品能力先在最接近目的的 index 或最终入口作用域写出并真实验证最短具体实现；生产结果已经具体出现后，才按实际数据边界更新 tree 并切出唯一纯生产者。生产者一旦形成正式边界，后续消费者必须等待它完整交付，不能继续内联复制或补偿。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "具体业务总成的实现目标是让 `index` 或既定最终入口真实交付成品，不是完成支撑它的 Class。先在 `index` 建立最终调用方实际使用的数据和方法；只有该具体实现已经出现必须独立维护的数据切片、状态、生命周期或不变量时，才从中切出 Class 或协作文件。Class 完成、测试通过或代码很多都不能单独计为项目进度。",
