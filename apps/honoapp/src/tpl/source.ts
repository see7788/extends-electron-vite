      ...
      sections: [
        {
          title: "正向业务链",
          items: [
            "先用一句话确定真实结果，核对生产者、消费者、owner、公开语义、消费责任和验收能够同时成立，再确定唯一业务入口。发现缺项或冲突时，先在已授权范围内读取、验证并查明；能够由既有事实唯一确定，或只属于不改变目标结果、公开契约、调用方责任和外部副作用的普通实现细节时，parent 直接作出最小技术决定并继续推进。只有无法查明且不同选择会改变这些根部事实时，才停止依赖该决定的路线并向 parent 报告，不得用参数、类型、Class 或配置掩盖待决事项。入口可以是 class 公开主方法、普通函数、高阶函数、模块方法、route handler、store action、命令或事件处理器。",
            ...
            "原子能力内部完成本动作必需的技术调用、后置校验和资源清理；入口显式编排业务顺序和合法分支，禁止通过重试、兜底、恢复、兼容或连续 catch 把失败改写成成功。",
          ],
          code: {
            language: "ts",
            content: [
        ...
        },
        {
          ...
          items: [
            ...
            "消费者只使用生产者已经公开的正式入口。已有配置 owner 时，禁止建立平行常量、静态状态文件、环境变量入口或消费者私有副本；消费者所需的最终形态由 owner 提供只读投影，不暴露内部 store 或要求消费者继续组装半成品。公开能力不足时，向方先生报告缺失事实、直接证据、影响范围和推荐方案；只有当前任务明确授权生产者改造时，才同步修改生产者及全部真实消费者。",
        ...
        },
        {
          title: "配置来源边界",
          items: [
            "只有项目既有配置边界或方先生明确要求使用环境变量时，才由真实配置 owner 集中读取 `process.env`，并从既定来源构造完整、带类型、可直接消费的最终值；其他对象只接收已经确定的配置或 owner 数据。同一事实只能由生产者解释一次，消费者不得用 `||`、`??`、默认参数或替代常量补齐配置。分支语法只处理契约中真实存在的多个状态并穷尽每个状态；未知状态在运行时抛出准确异常，契约尚未定义时向方先生报告并等待决策。布尔字段只有在下游 API 原样消费该布尔值时才能公开。",
          ],
        },
        {
          title: "输入、确定值与失败",
          items: [
            ...
            "不存在是合法结果时返回明确的不存在状态；当前操作要求对象或输入存在时，在最早能准确说明问题的边界抛出保留原始 cause、错误类型或可定位证据的异常，不用空值、失败布尔值、旧缓存、日志或替代值继续运行。",
            "Codex 无法从必填输入、确定性派生或已确认 owner 数据唯一确定业务事实时，停止依赖该事实的实现，向方先生说明缺失事实、直接证据、影响范围和推荐方案，等待方先生决策；不得自行增加默认值、备用实现、兼容分支或隐藏配置。",
          ],
        },
        {
          title: "需求语义闭环",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "复用原子方法时，入口仍显式保留当前业务步骤和合法分支的处理顺序；每个分支必须处理完整，未识别状态在运行时抛出准确异常，设计阶段尚未定义的状态由 parent 向方先生说明证据、影响和推荐方案并等待决策。只包裹一个调用点且没有独立状态、不变量或生命周期的 helper 直接内联。",
        ...
        },
        {
          title: "方法、返回与生命周期",
          items: [
            "对象在原子方法内完成本动作必需的技术后置校验和资源清理，并保留 cause 抛出准确异常；入口只进入既有契约中由明确输入或状态选定的合法分支，禁止捕获失败后改走另一能力。",
      ...
      sections: [
        {
          ...
          items: [
            ...
            "消费项目调用现有自有库时，生产者库完全只读：禁止新增、删除、移动或改名文件，禁止修改源码、类型、导出、参数、固定值和 package 元数据。公开能力不足时，parent 向方先生报告缺失事实、直接证据、影响范围和推荐方案，取得对生产者库及具体范围的明确授权后才能修改。任务本身明确开发该库时，只按授权 ownership 修改；只改库内部实现且公开用法不变时不修改消费者。",
          ],
        },
        {
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "网络配置名称必须表达真实协议或职责，不得借用另一个库、产品或不兼容协议的服务名称制造可替换假象。协议和生产者确定后，由生产者直接公开最终 URL、协议字符串或可直接传入客户端的配置对象；真实存在运行期协议选择时，由同一生产者穷尽处理每种协议并产生最终配置，未知协议在运行时抛出准确异常。协议身份或生产者尚未确认时，parent 向方先生报告缺失事实、直接证据、影响范围和推荐方案并等待决策，不以同名配置直接接线。禁止公开 `secure`、`tls` 等中间判断值，让消费者重新推导 `ws/wss`、`http/https` 或 `stun/stuns`；只有下游 API 原样消费该布尔值时才能公开。",
          ],
        },
        {
          title: "后端网络 - Hono API",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "任务存在要求冲突、必要生产者缺失、契约未定义、授权不足，或方案会改变目标结果、公开契约、调用方责任、数据处理、外部副作用或不可逆范围时，parent 不以抛出错误或长篇解释代替推进，也不得自行补充业务定义；按结论、根因与直接证据、影响范围、推荐方案、其他可行方案及差异、确认后立即执行的下一步向方先生提交一个明确的 `[?]`，只停止依赖待决事实的路线并继续其他工作。方先生确认后视为对应方案的实施授权，parent 立即恢复执行，不重复讨论或只返回新计划。",
        ...
        },
        {
          title: "观察者启动与报警",
          items: [
            "每个新会话收到方先生第一条消息时，只要运行时实际提供 watcher 角色和 watcher.definition.GET，parent 就先调用该接口取得 MCP 维护的完整 watcher 提示词，再将返回内容作为唯一启动任务，使用不继承对话历史的新实例创建唯一的会话级 watcher；普通追问和连续对话不重复创建。parent 不自行编写、补充或解释该提示词。任一能力未提供、接口失败或创建失败时，parent 立即如实告知方先生。",
        ...
        },
        {
          ...
          items: [
            ...
            "需要方先生决定时只提出真正阻塞推进的一个明确问题，并使用 `[?]` 写明待确认事实、直接证据、影响范围、首选推荐及理由、其他可行选择及差异、确认后立即执行的下一步；parent 不得用默认值、兜底、兼容或自行新增分支替代该决定，也不得把普通实现细节交给方先生。方先生确认后直接进入 `[ ]` 或 `[~]` 并立即执行；不依赖该决定的任务持续推进。",
        ...
        },
        {
          ...
          items: [
            ...
            "未完成事项存在安全、已授权且不依赖方先生新决定的下一步时，parent 必须继续处理，禁止以解释、建议、总结、计划或阶段性成果提前结束；只有需要方先生决定时提交带首选推荐的 `[?]`，或存在真实外部阻塞时提交写明解除条件的 `[!]`。不能继续的事项同时更新项目文档中的可审计工作流，写清阻塞原因、下一步动作和相关文件，不得只散落在回复里。",
            ...
            "只有所有目标项均已处理且完成必要验收后才使用“完成了”或“已处理完”。仍有任务且无需方先生决定时继续实施；确需决定时给出首选推荐和确认后立即执行的下一步，真实外部阻塞时给出卡点与解除条件。正确的分析、解释和建议本身都不构成交付，也不能成为停止推进的理由。",
          ],
        },
        {
          title: "文档使用边界",
