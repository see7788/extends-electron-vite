  ...
  skills: {
      ...
      sections: [
        {
          title: "目的与结果准入",
          items: [
            "任何任务先从方先生要实现的目的开始，把模糊目的逐级具象为边界外可观察、可消费、可真实验证的成品结果，再确定项目总成或最终入口必须提供的数据与公开方法。不得把完成某个文件、Class、方法、store、类型、接口或技术层本身当作目的；它们只在成品结果已经需要时成为实现手段。",
            "实施方向固定为 `目的 → 成品 data → index/最终入口的完整类型 → 递归生产切片（data + dataAction）→ 实现入口消费者`。类型可以一次表达整个项目成品的数据和公开方法；任何文件、Class、方法、helper、action、配置层或 export 如果不在这条路径上，就没有出生资格，不得先构思代码结构再把它接到目的上寻找用途。",
            "顶层项目总成可以主要由一个 Class 承载：完整类型定义成品形态，constructor 只建立已经由类型确定的生产者成员、owner 和固定依赖，不执行消费业务。生产切片存在于成员的数据责任中，不要求每个切片建立一个 Class；只有成员真实拥有必须独立封装的状态、生命周期或不变量时才拆出 Class。",
            "从最终入口开始保持最短正向主线，每一层必须新增完整项目数据、真实业务规则、独立状态、生命周期或不变量；连续层只代理、转发、改名、传递上一步结果或遮蔽真实调用时，不解释为架构层次，直接删除中间层。主线层数不设形式上限，但实现复杂度不能来自无数据意义的间接调用。",
            "README tree 是目的与结果当前最具体的实体蓝图，但低于真实目的、成品结果和运行证据。tree 节点不构成实现许可；具体实现证明某节点不在目的路径、没有数据结果或没有消费者时，立即停止该节点，先从 tree 删除或改正，再继续实现。禁止为了忠实蓝图把垃圾节点做完，也禁止源码静默丢弃后保留失真的 tree。",
          ],
        },
        {
          ...
          items: [
            ...
            "项目数据必须在当前技术步骤之外仍具有明确业务意义：它被真实业务直接消费，或由 owner 持续维护状态、生命周期或不变量。只在一个实现内部临时存在、仅传给下一技术步骤、离开当前调用就没有独立意义的局部变量、中间结果和转换形态不是项目数据，不得据此建立切片、生产者、类型、方法或文件。",
            "生产以数据切片为边界，消费以具体业务为边界；生产切片成套定义 data 与 dataAction，消费者自由组合已经真实可用的 data 和 action，并在自身作用域封装消费规则。每份 data 及其 dataAction 只能归属唯一具体生产者；共同保障该数据的数据字段、状态、动作、不变量和生命周期归该生产者。",
            "从成品 data 找手：入口仍缺少的每份 data 都形成一个生产切片；切片的 data 与使它真实成立、变化并保持可消费的 dataAction 必须在设计中同步、成套定义，二者缺一时切片设计未完整，不得把孤立 data 或孤立 action 冒充完整切片。实现允许按当前目标逐项推进：已经真实成立的 data 可以立即消费，尚未实现的 dataAction 保持明确未完成，只阻塞实际调用它的业务链，不阻塞其他已有 data、action 或可运行入口。dataAction 内部仍缺下级 data 时，对所缺数据同步建立下一套 `data + dataAction`，直到已有环境、外部协议或已完成切片；递归层数不设上限。方法只是 dataAction 的实现载体，不是切片依据。",
            "消费关系不属于切片。消费者直接消费一个或多个切片的完整数据，读取顺序、组合、判断、交互和业务规则留在当前消费作用域；跨切片消费是常态，不构成合并切片、共享 owner、修改生产边界或创建公共组合层的理由。",
            "生产者是实际创建、更新、验证并维护切片数据不变量的一方；只转发、改名、包装、缓存、映射或调用其他对象的结构不是该数据的生产者。生产者只负责数据如何真实成立并保持可消费，不预测或封装具体业务怎样使用；消费者只使用生产者公开交付的切片数据，不介入内部生产步骤、不解释内部状态、不补齐生产责任，也不继续组装半成品。",
            "消费者的结果只有在当前消费场景之外仍被真实业务直接消费，或开始由自身持续维护状态、生命周期或不变量时，才成为新的项目数据，当前消费者才相对成为新切片的生产者；局部派生值、展示格式、下一步骤输入和一次性组合结果仍属于当前消费规则。",
            ...
            "文件、Class、对象和 package 的角色由它围绕的真实对象决定，不做绝对形式分类：纯生产工具围绕一份切片完整交付数据，具体消费者围绕一项业务直接组合多个生产者，项目总成围绕整个成品组合并提供业务需要的数据与能力。不得为了结构纯粹强迫具体消费者或项目总成拆散，也不得借业务总成之名让内部工具、技术步骤和无真实用途的成员混入公开面。",
        ...
        },
        {
          title: "生产者完整性准入",
          items: [
            "先由目的和成品 data 确定 index/最终入口的完整类型，再按所缺 data 递归确定每套同步出现的 `data + dataAction`，并在项目总成 constructor 中建立完整类型已经要求的生产者成员；不得从类名、方法名、技术步骤或想象复用推导额外能力。实现按当前一句话目标选择最短闭合链，从该链递归最深处逐套生产并真实验证，再实现只消费这些已就绪成员的入口；其他 dataAction 可以明确保持未实现，不得用等待全部 action 的大瀑布阻止项目运行和继续推进。",
            "实现任何消费者前，先逐项列出该业务实际读取的 data 和调用的 dataAction，并把每项定位到唯一切片、具体生产者和正式公开成员。当前消费链使用的成员已经真实就绪即可编写消费者，不要求同切片或整个项目的其他 action 全部实现；已有 data 可被只读消费。任意必需成员缺失、形态不完整或需要消费者理解生产步骤时，只停止依赖该成员的路线：当前任务已授权生产者时回到对应生产者补齐并真实验证；未授权时按全局错误与决策规则提交缺口。",
            "生产者缺口不得通过新增参数、配置、环境变量、默认值、缓存、wrapper、adapter、helper、消费者私有副本、重复查询或跨层访问绕过。能够从既有生产者数据确定性取得的值由生产者直接交付最终形态；生产者尚不能交付时修改生产者，不把缺口转嫁给业务调用方。",
            "消费者开始出现重复参数、数据转换、生产状态判断、补值、跨层查询或多层转发时，默认视为生产者可能没有完整交付数据；先沿所需数据反查唯一切片和具体生产者，确认生产责任完整后，才能判断剩余逻辑是否真属于当前消费业务。",
            "消费者只封装当前业务怎样读取、组合和使用完整切片数据。消费者复杂度只能来自真实业务规则；如果删除生产补偿、数据搬运和转发后没有独立业务规则，消费者层本身不成立，直接在最接近的真实消费作用域使用切片数据。",
          ],
        },
        {
          title: "数据与生产者保护",
          items: [
            "方先生已经定义或现有项目已经稳定使用的项目数据、字段、类型、默认值、持久化形态、切片边界、唯一生产者、owner、文件路径、公开成员、返回语义和生命周期属于设计不变量。没有针对对应数据或生产者的明确修改要求，不得以实现消费者、修复其他模块、适配调用方、统一风格、重构、复用、简化或生产级为由改变。",
            "实现消费者不构成修改生产者的授权；普通项目功能、调用库、接口接线和消费者 bug 修复也不自动授权改变既有生产者。只有方先生明确要求修改该生产者、其切片数据或公开契约时才进入生产者写入范围；否则生产者只读，发现缺口立即停止依赖路线并按全局错误与决策规则提交证据和建议。",
            "取得生产者修改授权后，先在 tree 中写明要补齐的具体切片数据、生产结果和受影响消费者，再只修改该缺口。未被目标点名的数据字段、成员、路径、状态、默认值、错误语义、生命周期和其他消费者保持不变；不得借补一个字段或方法重命名、移动、合并、拆分、替换或重写整个生产者。",
            "生产者内部实现可以在明确授权范围内改变，但必须保持未授权的项目数据和公开生产语义完全一致。实现便利、类型自洽、测试通过、减少代码、框架惯例和新消费者更好调用都不能证明数据或生产者变化合理；无法同时保持时先提交差异由方先生决定。",
          ],
        },
        {
          title: "正向业务链",
          items: [
            "根模型确定后，先用一句话或一个完整类型表达入口业务怎样完成成品 data；不得把句中的动作词直接切成方法。逐项找出尚未成立的 data，为每项同步定义同一切片的 `data + dataAction`，并以整套切片递归到已有生产能力，再核对每层生产者、直接消费者、owner、公开语义、责任和验收能够同时成立。入口可以是 class 公开主方法、普通函数、高阶函数、模块方法、route handler、store action、命令或事件处理器。",
            ...
            "主线需要下级数据时只调用已经完整交付该数据的直接生产者；缺少生产者或公开数据时进入生产者完整性准入，不得先创建技术能力、工具对象或中间层再反向拼装主线。",
            "消费入口从环境生产者取得初始数据，再让前一生产者的完整结果成为下一生产者的明确输入，直接写出真实数据流。没有独立数据、状态、生命周期、不变量或新业务规则的调用片段不得另起 `publish`、`deploy`、`run`、helper 或代理名称遮蔽调用链。",
            "挂在某个生产者名下的方法，如果跨切片组合其他独立生产者完成业务结果，却没有生产并维护属于自身 owner 的纯粹数据，属于套娃黑盒无意义转发；删除该方法并把调用链移回真实业务消费者。生产者只能调用完整交付自身切片所必需的直接依赖，不得借生产步骤隐藏同级业务消费。",
            "主线完成后以真实文件、状态、响应、进程或生产者结果验证；局部函数返回、日志或无异常只证明该调用结束，不替代业务结果成立。",
            "一个函数或方法直接完成一个完整生产场景或消费场景；读取、校验、安装、启动、修改、验证、清理等共同保障同一结果的技术步骤保持在该场景内部，不按步骤拆成方法、helper、工具 Class 或文件。只有其中一部分独立生产项目数据、封装独立业务消费规则，或真实拥有独立状态、生命周期或不变量时，才形成新的封装场景。",
            "生产方法完整交付切片数据和必要后置验证，消费入口显式完成业务顺序和合法分支；状态已同步且仍由同一实例继续承担时返回 `this`，否则直接返回消费者需要的最终数据。",
          ],
        },
        {
          title: "命名与参数",
        ...
        },
        {
          title: "最小作用域与真实实现",
          items: [
            "函数、方法和文件只按完整生产场景或消费场景确定边界，代码行数永远不构成拆分证据。一个场景无论多少行都先保持语义内聚；使用正常换行、局部变量、代码块、分支和必要注释组织阅读。只有直接上级真实需要的独立 data、业务消费规则、状态、生命周期或不变量才允许形成新边界；生产侧以设计上同步成套的 `data + dataAction` 为单位递归，层数不设上限，每层都必须有明确 data、dataAction 契约、唯一 owner 和已经成立的直接消费者。dataAction 可以按当前业务需要分期实现，但未实现状态必须真实可见，不得冒充已交付。",
    ...
    [nodes.codeClassStyle]: {
      ...
      sections: [
        ...
        {
          title: "从切片数据到具体 Class",
          items: [
            `Class 只能承载已经由 ${nodes.codeStyle} 确定的完整生产场景、消费场景或项目总成。生产与消费 Class 从切片数据或业务结果开始；项目总成 Class 从成品完整类型开始，并用成员承载已经确定的生产者和消费者。禁止从抽象契约、基类、工具 Class、技术步骤或无目的成员骨架开始向上拼装业务。`,
            "生产 Class 直接拥有并完整生产一份切片数据，维护相应状态、不变量和生命周期；消费 Class 只在自身公开主方法中组合完整切片并封装业务规则。一个 Class 同时不能用技术分层掩盖生产与消费责任，确有两种独立场景时按根模型重新确认边界。",
            "项目总成 Class 围绕整个成品而不是单个切片成立，可以在一个 Class 中组合多个异质生产者和消费者；这不是 facade。其成员必须由成品类型或真实消费数据直接要求，成员内部仍保持各自生产边界；不得为了形式拆成多层 aggregate、runtime、manager 或 service Class。",
            "下级成员 Class 只在它已经独立生产项目数据，或真实拥有独立状态、生命周期、不变量或协议转换时成立；读取、校验、转换、安装、启动、验证和清理等只为当前结果服务的步骤留在当前方法，不得各自升级为工具 Class 或中间能力层。",
            "抽象类和基类不是实现起点。只有多个已经真实存在的具体生产 Class 具有相同公开语义、owner、变化原因和共同不变量，并且调用方必须通过同一契约使用它们时，才从这些具体事实归纳抽象契约；不得先写抽象层再制造具体实现履约。",
            "Class 层次没有预设数量；缺少独立项目数据、消费规则、状态、生命周期或不变量的层直接省略。只能转发、改名、包装或重复直接依赖实现的 Class 必须删除、内联或合并。",
            "验收时从项目数据沿 tree 逐层指出每个 Class 完整生产的数据或封装的业务消费规则、首个真实消费者、独立不变量和变化原因；无法闭合任一项时不得保留该 Class。",
          ],
        },
        {
          title: "入口与业务主线",
          items: [
            "先确定入口 Class 的公开主方法；生产主方法完整交付切片数据，消费主方法按业务顺序直接读取并组合成员生产者的完整数据、同步自身状态并返回结果，这条扁平调用链就是主线。",
            "主方法显式保留当前场景步骤和合法分支的处理顺序；每个分支必须处理完整，未识别或契约未定义的状态进入全局错误与决策规则。只包裹一个调用点、技术步骤或局部转换且没有独立项目数据、业务消费规则、状态、不变量或生命周期的 helper 直接内联。",
        ...
        },
        {
          ...
          items: [
            ...
            "项目总成 constructor 按完整项目类型一次建立已经确定的生产者成员，并让每个成员归属唯一切片；constructor 不调用生产方法、不执行消费者顺序，也不增加为了分类、转发或未来复用的成员。每个消费者只等待自身直接读取的 data 和调用的 dataAction 就绪，不等待无关 action；因此项目可以在完整业务尚未全部实现时保持可运行并继续扩展。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "具体业务总成的实现目标是让 `index` 或既定最终入口真实交付当前一句话目标，不是完成支撑它的 Class。先用完整类型定义 `index` 的成品 data、dataAction 和公开方法，再由顶层总成 Class 的 constructor 建立类型已经要求的生产者成员；每条消费链只等待自己使用的生产成员就绪，不要求其他 action 完成。支撑 Class 完成、测试通过或代码很多都不能单独计为项目进度。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "已定义 Actions 根成员只按已经成立的生产者对象或业务消费场景组织；不得为了可读性、分类、形式对称或技术步骤加深对象路径。新的路径必须能指出独立项目数据或完整业务消费规则，否则保留在当前 action 内。",
            ...
            "跨文件方法只有在当前完整生产或消费场景必须由边界外直接调用时，才通过方先生已经定义的 Actions 或对象公开；局部技术步骤直接留在当前 action，不得改名为私有方法或内部对象来规避完整场景准入。",
            "多个 action 只有分别生产独立项目数据或封装独立业务消费规则时才可用递归功能对象组织；末端成员保持当前作用域所需的最短明确名称，不得用路径分类把一个完整动作拆成多层调用。",
          ],
        },
        {
          title: "Action",
          items: [
            ...
            "仓库 Actions 只保留生产切片数据、维护既有状态不变量或被真实业务入口消费的完整动作；不要求必须由 React 直接消费，但必须能指出具体生产结果或业务消费者。禁止为了实现步骤或方法方便新增 action、内部方法和未授权状态字段。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "完成前逐项审计本轮新增的文件、class、类型、public/protected/private 成员、局部函数、helper、hook、action、内部对象路径、配置层和 export；每项必须指出它完整生产的项目数据或封装的业务消费规则、具体生产者或消费者、独立状态/生命周期/不变量、最小作用域以及提升或导出依据。只能承载技术步骤、转发、改名、分类或可读性且缺少完整场景证据的结构必须删除、内联或合并；改成 private、藏入 action 或不写进 tree 都不能豁免。",
            "从项目总成、进程入口和边界外真实业务反向遍历 tree 与源码调用链；无法沿消费关系到达的文件、Class、公开成员或数据切片判定为孤儿。代码量、实现复杂度、历史投入、已经写完和未来可能使用都不能证明生产者成立；本轮新增的孤儿直接删除或内联，既有孤儿未获删除授权时保留原状并向方先生提交位置、影响和建议，不得把它塞进 `index`、增加转发或制造消费者为其续命。",
            "任务触及既有生产者时，修改前记录对应 tree 节点、项目数据字段、public/protected 成员、公开返回和真实消费者，修改后逐项比较；任何未被方先生明确要求的数据、owner、路径、成员、语义或生命周期差异都必须从本次改动中移除。消费者构建成功、类型通过或调用更简单不能替代生产者不变量验收。",
