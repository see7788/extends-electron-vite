...
// const chatRouter = new Hono().basePath("/chat")
//   .get("/state", (ctx) => {
...
//     const runtimeActions = store.getState().runtimeActions;
//     if (!runtimeActions) throw new Error("Runtime actions have not been initialized");
//     store.setState(state => {
//       state.chat = chat;
//     });
//     store.getState().chatActions.defFactoryReplace({ workspacePath: runtimeActions.workspacePath })
...
//     const runtimeActions = store.getState().runtimeActions;
//     if (!runtimeActions) throw new Error("Runtime actions have not been initialized");
//     return ctx.json(store.getState().chatActions.agent.codexcli.defConfig({ workspacePath: runtimeActions.workspacePath }));
//   })
//   .post("/agent/codexcli", zValidator("json", store.getState().chatActions.inputSchema), (ctx) => {
//     const { prompt } = ctx.req.valid("json");
//     const runtimeActions = store.getState().runtimeActions;
//     if (!runtimeActions) throw new Error("Runtime actions have not been initialized");
//     return store.getState().chatActions.agent.codexcli.defChat({ prompt, workspacePath: runtimeActions.workspacePath })
//   });
