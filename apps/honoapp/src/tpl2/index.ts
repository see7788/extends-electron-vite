...
import type { Tpl2Store } from "./store";
...
type Tpl2Env = {
  Bindings: {
    api: Pick<StoreApi<Tpl2Store>, "getState">;
    mcp: { server: McpServer; transport: StreamableHTTPTransport };
    responseContentRead: (response: Response) => Promise<{ content: Array<{ type: "text"; text: string }> }>;
  };
};

const tpl2McpServersRegistered = new WeakSet<McpServer>();

const tpl2Router = new Hono<Tpl2Env>()
  .get("/tpl2/source", zValidator("query", workspacePathSchema), (ctx) => ctx.json(
    ctx.env.api.getState().tpl2Actions.sourceRead(ctx.req.valid("query").workspacePath),
  ))
  .put("/tpl2/source", zValidator("json", sourceInputSchema), (ctx) => {
    const { source, workspacePath } = ctx.req.valid("json");
    ctx.env.api.getState().tpl2Actions.sourceUpdate(workspacePath, source);
    return ctx.body(null, 204);
  })
  .post("/tpl2/output/filesStatus", zValidator("json", workspacePathSchema), (ctx) => ctx.json(
    ctx.env.api.getState().tpl2Actions.outputFilesStatus(ctx.req.valid("json").workspacePath),
  ))
  .post("/tpl2/output/materialize", zValidator("json", workspacePathSchema), (ctx) => {
    ctx.env.api.getState().tpl2Actions.outputMaterialize(ctx.req.valid("json").workspacePath);
    return ctx.body(null, 204);
  })
  .post("/tpl2/output/rebase", zValidator("json", workspacePathSchema), (ctx) => {
    ctx.env.api.getState().tpl2Actions.outputRebase(ctx.req.valid("json").workspacePath);
    return ctx.body(null, 204);
  })
  .all("/tpl2-mcp", async (ctx) => {
    const { api, mcp, responseContentRead } = ctx.env;
    if (!tpl2McpServersRegistered.has(mcp.server)) {
      mcp.server.registerTool("tpl2.source.GET", {
        title: "读取 Codex 模板源码",
        ...
        undefined,
        { api, mcp, responseContentRead },
      )));
      mcp.server.registerTool("tpl2.source.PUT", {
        title: "更新 Codex 模板源码",
      ...
      mcp.server.registerTool("tpl2.output.filesStatus.POST", {
      ...
      mcp.server.registerTool("tpl2.output.materialize.POST", {
        title: "物化 Codex 模板",
      ...
      mcp.server.registerTool("tpl2.output.rebase.POST", {
      ...
      tpl2McpServersRegistered.add(mcp.server);
    }
