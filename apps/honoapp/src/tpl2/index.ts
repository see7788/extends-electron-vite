...
mcp.requestToolRegister({
  ...
  request: ({ workspacePath }) => tpl2Router.request(
    `/tpl2/source?${new URLSearchParams({ workspacePath })}`,
  ),
});
mcp.requestToolRegister({
  name: "tpl2.source.PUT",
  title: "更新 Codex 模板源码",
  description: "验证并保存指定工作区的 Codex 模板 TypeScript 源码；在已经取得完整新源码并需要替换当前模板时使用；需要现存工作区绝对路径和完整源码；成功后返回 null；会覆盖该工作区已保存的模板源码，但不会立即物化文件。",
  inputSchema: sourceInputSchema,
  annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false },
  request: (json) => tpl2Router.request("/tpl2/source", {
    method: "PUT",
...
mcp.requestToolRegister({
  name: "tpl2.output.filesStatus.POST",
  ...
  inputSchema: workspacePathSchema,
  annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  request: (json) => tpl2Router.request("/tpl2/output/filesStatus", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(json),
  }),
...
mcp.requestToolRegister({
  name: "tpl2.output.materialize.POST",
  title: "物化 Codex 模板",
  description: "把指定工作区当前保存的 Codex 模板写入该工作区的 .codex 配置及受管文件；在模板已经确认并需要实际生效时使用；需要现存工作区的绝对路径；成功后返回 null；会创建或更新受管文件。",
  inputSchema: workspacePathSchema,
  annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false },
  request: (json) => tpl2Router.request("/tpl2/output/materialize", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(json),
  }),
...
mcp.requestToolRegister({
  name: "tpl2.output.rebase.POST",
  title: "重建 Codex 模板物化基线",
  description: "在安全检查通过后，把指定工作区当前受管文件记录为新的 Codex 模板物化基线；仅在确认现有受管文件应成为后续比较基准时使用；需要现存工作区的绝对路径；成功后返回 null；会更新物化状态记录。",
  inputSchema: workspacePathSchema,
  annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false },
  request: (json) => tpl2Router.request("/tpl2/output/rebase", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(json),
  }),
