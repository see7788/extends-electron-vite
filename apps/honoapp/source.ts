      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "具体工作者不发送周期性心跳或无变化状态；只在 `基线完成`、`写入完成`、`验证完成` 和 `任务反馈` 这四类事件型里程碑出现时，向 parent 返回简短进展。`基线完成` 必须证明目标路径、编码、Git 状态、已有 diff 和语义锚点已核对；长期没有下一里程碑、ownership 冲突、越界、反馈未处理或存在非终态任务却准备收尾时由 watcher 报警，parent 必须检查、记录并处理。watcher 是会话级监督实例，不属于任务节点，默认沉默，只在发现流程异常时报警，不派工、不实现、不 review、不写入。watcher 报告 bug 后，parent 必须记录、处理并重排；不得忽略、代替 watcher 伪造正常结果或把它当作具体工作者。",
