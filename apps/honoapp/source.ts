      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "同一 pnpm TypeScript package 内的跨文件 import 必须以当前 `package.name` 为根并拼接被导入源码的真实磁盘目录；禁止使用 `./`、`../`、绝对路径、`file:` 或 `link:`，也禁止为 TypeScript 源码补写 `.js` 后缀。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "具体工作者不发送周期性心跳或无变化状态；只在基线完成、写入完成、验证完成和任务反馈这四类事件型里程碑出现时，向 parent 返回简短进展。长期没有下一里程碑时由 watcher 报警，parent 必须检查并处理。watcher 是会话级监督实例，不属于任务节点，默认沉默，只在发现流程异常时报警，不派工、不实现、不 review、不写入。watcher 报告 bug 后，parent 必须记录、处理并重排；不得忽略、代替 watcher 伪造正常结果或把它当作具体工作者。",
            "每份任务信封保持短而完整，只含完成当前任务必需的目标、精确 ownership、直接调用上下文、要求动作、停止边界、必要 skill 和验收条件；不得附带完整对话、parent-workflow、总台账、其他任务或长背景。直接调用上下文只写已确认的入口、关键调用链、精确入参和返回值、副作用及错误边界。",
            "parent 在派工前先完成足以写出短信封的调查并拆分 ownership；互不依赖且 ownership 不重叠的任务按当前可用槽位并行创建实例，存在依赖或写入范围重叠时串行。每个实例必须有可区分标识、运行时模型标识和已声明依赖。",
            "稳定对象目录可以绑定可复用维护者；维护者显示标识、任务信封和 ownership 统一使用工作区相对路径，运行时标识不支持路径字符时只作机械替换。同一目录内的写入必须串行，parent 派工时必须注入目录外生产者、消费者和验收上下文。跨目录任务拆成具有明确依赖的节点，不同目录在依赖满足且 ownership 不重叠时按当前可用槽位并行；复用维护者前必须重建该目录相关目标文件的当前基线。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "AI 为调查、调试、验证或事故恢复产生的非长期交付物，包括一次性脚本、诊断源码、日志、截图、抓包、临时配置、缓存、转换文件和候选恢复文件，只能写入当前仓库根目录 `.log/`；按任务建立可识别的子目录或文件名前缀，禁止散落在仓库根、业务目录、源码目录或用户目录。",
            ...
            "没有放权时只能修改用户指定范围；删除非本轮创建的文件仍需确认。`.log/` 是临时产物的唯一落点，不是绕过文件或目录创建授权的通道。",
          ],
          code: {
            language: "diff",
            content: [
