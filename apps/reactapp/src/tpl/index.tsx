...
const renderConfigToml = (tpl: Tpl) => [
  ...
  "",
  ...(tpl.configToml.features.hooks ? (tpl.configToml.hooks.UserPromptSubmit?.flatMap(hook => ["[[hooks.UserPromptSubmit]]", `hooks = [{ type = ${JSON.stringify(hook.type)}, command = ${JSON.stringify(hook.command)}, timeout = ${hook.timeout} }]`, ""]) ?? []) : []),
  ...(tpl.configToml.features.hooks ? (tpl.configToml.hooks.Stop?.flatMap(hook => ["[[hooks.Stop]]", `hooks = [{ type = ${JSON.stringify(hook.type)}, command = ${JSON.stringify(hook.command)}, timeout = ${hook.timeout} }]`, ""]) ?? []) : []),
].join("\n").trimEnd() + "\n";
...
export default function CodexTplPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      sourceSave({ source, workspacePath })
        .then(() => {
  ...
  }, [source, sourceSave, sourceSaveStatusChange, sourceSaveTickNext, tpl, workspacePath]);
  useEffect(() => {
