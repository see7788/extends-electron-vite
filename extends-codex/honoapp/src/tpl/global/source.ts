    ...
    sections: [
      {
        title: "总纲",
        items: [
          `当前主 Codex（以下简称 parent）仅在当前会话的首条方先生消息时，先加载 ${nodes.parentWorkflow}；普通追问、澄清和连续对话不得重复检查。`,
          `parent 必须且仅由自身加载 ${nodes.parentWorkflow}、${nodes.docStyle} 与 ${nodes.templateService}；watcher 只加载 ${nodes.watcherWorkflow} 和会话运行时事件。`,
    ...
    [nodes.scopeStyle]: {
      ...
      sections: [
        ...
        },
        {
          title: "显式配置边界",
          items: [
            "默认厌恶并禁止通过 `process` 传递业务或项目参数，包括直接或间接使用 `process.argv`、`process.env`、`process.cwd()`、`process.execArgv`、`process.title` 及其包装、转发或全局读取；编译期已知值留在真实 owner，运行时值优先由有类型 config、配置文件、公开 CLI 契约、IPC contract 或 owner 状态显式传递，不得把当前进程状态伪装成调用参数。",
            "只有外部宿主或第三方协议确认无法绕开时才允许 `process.env`：唯一读取点是项目主 `store.ts` 文件顶部的 `runtimeAction` 根成员；其他 `store.ts`、`index.*`、切片仓库、路由、组件和业务模块均不得读取、包装、动态访问或透传原始环境值。公开 CLI 契约只允许项目唯一 `index.*` 解析 `process.argv`。该例外必须说明不可替代的外部协议原因与唯一读取位置。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "主仓库不定义业务类型、业务常量、schema、工具函数或运行时对象；唯一例外是本项目运行时元数据的 `runtimeAction` 根成员，其规则和示例见「运行时根成员」。",
        ...
        },
        {
          title: "运行时根成员",
          items: [
            "项目确实需要读取 `process.env` 时，项目主 `store.ts` 文件顶部必须定义唯一的 `runtimeAction`；它是主仓库的根成员，不持久化，不下沉为切片，也不拆成 helper。",
            "`runtimeAction` 只暴露项目实际运行时契约需要的明确、无参读取方法；`workspacePathGet()`、`hostnameGet()`、`portGet()` 只是三项都需要时的示例，不是所有项目的固定字段。方法直接返回已约定存在的值，禁止空值兜底、返回 undefined、通用 get/set、原始环境对象或向业务层传递运行时参数。",
            "主仓库初始值只返回同一个顶部 `runtimeAction` 对象；路由、组件、切片和业务模块只调用这些方法，禁止出现 `process.env` 或第二处环境变量映射。",
          ],
          code: {
            language: "ts",
            content: [
              "// 示例：当前项目的运行时契约确有路径、主机和端口三项时。",
              "type runtimeAction_t = {",
              "  workspacePathGet: () => string;",
              "  hostnameGet: () => string;",
              "  portGet: () => number;",
              "};",
              "",
              "const runtimeAction: runtimeAction_t = {",
              "  workspacePathGet: () => process.env.WORKSPACE_PATH!,",
              "  hostnameGet: () => process.env.HOSTNAME!,",
              "  portGet: () => Number(process.env.PORT!),",
              "};",
              "",
              "type store_t = {",
              "  runtimeAction: runtimeAction_t;",
              "};",
              "",
              "// 主 store.ts 的 initializer 根层：",
      ...
      sections: [
        {
          ...
          items: [
            ...
            "普通项目先检查 VS Code 是否安装 `see7788.extends-codex-vscode`；未安装时明确提示方先生安装，不猜测端口、服务地址、进程输出或内部状态。",
            "插件已安装时由方先生通过插件启动服务；服务启动完成后自动物化当前工作区的项目 AGENTS.md、config 和 skills。AI 不直接修改生成的 `.codex` 产物，也不需要取得 Hono 服务地址。",
          ],
        },
        {
          title: "接口与物化",
