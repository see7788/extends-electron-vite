...
- [x] [05:04] T-093 方先生发现：三个消费真实 `.ts` 包路径的 tsconfig 只启用了 `allowImportingTsExtensions`，没有 TypeScript 要求的 `noEmit`、`emitDeclarationOnly` 或 `rewriteRelativeImportExtensions`，导致编辑器配置报错。确认三个 package 均由 tsup/electron-vite 构建后补充 `noEmit: true`；honoapp 与 userapp-electron-main 类型检查通过，admin-electron-main 仅剩既有的 `extends-electron/main/loginState` 模块缺失，修正已提交为 `f17ff16`。
- [x] [04:41] T-092 方先生确认：每个任务或项目只保留一个最终生效的 `todo-mcp`。同名配置按当前任务/启动参数覆盖、离当前目录最近的项目配置、用户级配置依次取最高优先级；项目级优先于全局，同名配置不合并、不重复启动。规则已提交为 `15277dd`，完成物化并核对生成结果。
- [~] [04:23] T-091 方先生确认：长期工作者按对象目录分片维护，实例数量与稳定目录数量相关，不随任务次数增长。每个目录由一个可复用维护者串行处理目录内写入并积累局部记忆；parent 在短信封中补充当前任务所需的目录外生产者、消费者、接口变化和验收事实，维护者不得越过目录 ownership。不同目录按剩余运行槽位并行；跨目录任务由 parent 拆为有依赖的生产者和消费者节点。每次复用仍须重建当前文件基线，发现记忆与事实冲突立即停止并由 parent 重排。
	- [~] [04:37] parent：已把“目录维护者、目录内串行、跨目录拆分、外部上下文由 parent 注入、按可用槽位并行，以及维护者显示标识/任务信封/ownership 统一使用工作区相对路径”写入权威 `parentWorkflow` 并物化；等待新会话验证后完成。
- [~] [04:11] T-089 方先生确认：建立不依赖 `todo-mcp` 的当前协作策略，并在 `todo-mcp` 完成后沿用。严格区分角色定义、Agent 实例和运行时并发槽位；一份 workerLow、workerMedium、workerMax 定义可创建多个实例，复制同名 agent 配置不能增加槽位。parent 按记忆价值选择实例：新领域、目标或边界变化、旧信封错误或上下文污染时使用 `fork_turns:none` 新实例；同一任务续作或稳定领域且上下文仍准确时允许复用，复用前必须重新建立当前文件基线。实例记忆只作背景，不得覆盖当前事实。
	- [~] [04:37] parent：实例选择、禁止占位预热、运行槽位按平台真实能力计算、旧终态实例只作历史等规则已写入权威 `parentWorkflow` 并物化；等待新会话验证后完成。
	- [~] [04:37] parent：精确短信封规则已写入权威 `parentWorkflow` 并物化；`todo-mcp` 仍缺任务信封对象与接口，接口完成后再关闭该节点。
	- [~] [04:37] parent：四类事件型里程碑及 watcher 报警事实已写入权威 `parentWorkflow` 并物化；`todo-mcp` 仍缺里程碑时间和 watcher 异常接口，接口完成后再关闭该节点。
	- [x] [04:37] parent：已验收并改正为按任务记忆价值选择新建或复用实例，不设默认无记忆；槽位、实例、短信封、并行、里程碑和 watcher 异常反馈策略已写入、提交并物化。
- [~] [04:37] T-090 parent 发现：当前工作者完整加载必要技术 skill 和建立文件基线存在固定启动成本，这不是心跳或推理卡死。权威 `parentWorkflow` 已采用稳定目录维护者、复用前重建基线、短信封只交付必要 skill 与四类事件型里程碑；`todo-mcp` 仍需记录里程碑时间，避免仅以 `running` 判断进展。
- [~] [02:12] T-088 方先生要求：workerMax 完成 30 项全局模板修改后，对新 `source.ts` 再做一轮只读残余废话审查；逐项给出原文、作用域、是否改变可执行决策、是否已被明确规则覆盖、删除影响及删留建议。parent 独立复核：双方同意删除的再派工删除，双方同意保留的继续保留，判断冲突项把原文和理由交由方先生取舍；第二轮审查期间不得自行修改源码。
...
- [ ] [15:54] T-073 方先生确认：实现以现有 TodoTreeNode 层级为可见载体的 `todo-mcp` 协作闭环。方先生需求下展示 parent 的整体任务信封，parent 一次性为具体工作者建立带 nodeId 的任务信封；角色的真实进展、完成、阻塞、审查或异常均追加为对应节点的语义化反馈子节点，parent 再在反馈节点下继续派发修复、审查或后续延伸任务，使全过程无需依赖对话转述即可观察。
	...
	- [ ] [15:54] parent：实现 `todo-mcp` 的任务与反馈接口。parent 使用 create/assign/replan/status/accept；worker、workerLow、tokener 使用 report(nodeId, message, evidence)；服务端把 report 自动物化为带时间、agent 和固定“已反馈”状态的只读子节点，成功、失败和阶段进展都只在真实发生后生成，不预建空节点。
