...
- [~] [04:23] T-091 方先生确认：长期工作者按对象目录分片维护，实例数量与稳定目录数量相关，不随任务次数增长。每个目录由一个可复用维护者串行处理目录内写入并积累局部记忆；parent 在短信封中补充当前任务所需的目录外生产者、消费者、接口变化和验收事实，维护者不得越过目录 ownership。不同目录按剩余运行槽位并行；跨目录任务由 parent 拆为有依赖的生产者和消费者节点。每次复用仍须重建当前文件基线，发现记忆与事实冲突立即停止并由 parent 重排。
	- [!] [04:23] parent：待把“目录维护者、目录内串行、跨目录拆分、外部上下文由 parent 注入、两个剩余槽位在并行实施或分析/实施之间动态分配”写入权威 `parentWorkflow`，完成物化和新会话验证后解除阻塞。
- [~] [04:11] T-089 方先生确认：建立不依赖 TodoTree 的当前协作策略，并在 TodoTree/MCP 完成后沿用。严格区分角色定义、Agent 实例和运行时并发槽位；一份 workerLow、workerMedium、workerMax 定义可创建多个实例，复制同名 agent 配置不能增加槽位。parent 按记忆价值选择实例：新领域、目标或边界变化、旧信封错误或上下文污染时使用 `fork_turns:none` 新实例；同一任务续作或稳定领域且上下文仍准确时允许复用，复用前必须重新建立当前文件基线。实例记忆只作背景，不得覆盖当前事实。
	- [!] [04:11] parent：已确认不使用空任务、占位任务或长期 `running` 做预热；角色模型、推理强度和职责可以预设，任务到来时再按记忆价值创建或复用实例。旧终态实例只作历史，不参与运行槽位统计；当前运行时允许的并发数属于平台能力，parent 和常驻 watcher 均占槽位，剩余槽位用于实施或审查，复制 agent 定义不能扩容。阻塞于权威 `parentWorkflow` 尚未完成物化和新会话验证。
	- [!] [04:11] parent：已确认任务信封必须精确简短，一个信封对应一个可独立验收动作，通常为一个文件或一至三个必须同时变化的紧密文件；机械同构批量修改可以列明确文件清单。信封只交付目标、精确 ownership、parent 已确认的直接生产者/消费者或调用上下文、要求动作、停止边界、唯一必要 skill、验收方式和反馈证据；parent 不得把全仓调查、架构判断、skill 筛选和交付范围推导转嫁给实施工作者。阻塞于权威 `parentWorkflow` 尚未物化，TodoTree/MCP 任务信封接口尚未实现。
	- [!] [04:11] parent：已确认工作者不承担周期心跳，只在完成、真实阻塞或信封事实错误时反馈。进展采用事件型里程碑：`基线完成` 证明目标路径、编码、Git 状态、已有 diff 和语义锚点已核对；随后依次记录 `写入完成`、`验证完成`、`任务反馈`。长期没有下一里程碑、ownership 冲突、越界、反馈未处理或非终态收尾属于 watcher 报警事实。阻塞于 TodoTree/MCP 尚未实现里程碑事件、时间和 watcher 异常接口。
	- [~] [04:11] workerMax：已将槽位、实例、默认无记忆一次性工作者、长期领域记忆例外、短信封、并行和 watcher 异常反馈策略写入 `apps/honoapp/source.ts` 的 `parentWorkflow`，TypeScript、UTF-8/LF 与 `git diff --check` 已通过；待 parent 验收、物化并建立 Git 检查点。
- [!] [04:11] T-090 parent 发现：当前工作者每次完整加载技术 skill 和建立文件基线约产生一分钟固定启动成本；这不是心跳或推理卡死。后续需在不违反 skill 强制加载规则的前提下，评估由长期领域实例保留已加载领域规则，或由 parent 在短信封中交付最小可执行约束；TodoTree/MCP 应记录四类里程碑及其时间，避免仅以 `running` 判断进展。
